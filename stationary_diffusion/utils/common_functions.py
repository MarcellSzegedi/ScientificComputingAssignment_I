...
def reset_grid_wrapping(grid: np.ndarray) -> np.ndarray:
    ...
    Args:
        grid: Existing grid (2D numpy array) where every cell value represents a delta step in the discretised square
                field with side interval [0, 1].
...
def _checks_grid_shape(grid: np.ndarray) -> None:
    ...
    """
    if len(grid.shape) != 2:
        raise ValueError(f"Grid must be a 2D, while the current grid's dimensionality is {len(grid.shape)}.")
    if grid.shape[0] != grid.shape[1]:
        raise ValueError(f"Grid must be a square, while the current grid shape is {grid.shape}.")
