...
def gauss_seidel_iteration(grid: np.ndarray, trans_matrix: np.ndarray):
    ...
    Args:
        grid: Existing grid (2D numpy array) where every cell value represents a delta step in the
                discretised square field with side interval [0, 1].
...
def det_iter_transformation(n: int) -> np.ndarray:
    """
    Creates matrix 'A' from the linear equation system of Ax = b for the t+1 time step of the gauss_seidel iteration.
    'x' is the vector of variables in the grid in t+1.
    ...
    diag_1 = np.tile(pattern_1, (var_vector_len // len(pattern_1) + 1))[:var_vector_len-n]
    diag_2 = np.tile(pattern_2, (var_vector_len // len(pattern_2) + 1))[:var_vector_len-1]
